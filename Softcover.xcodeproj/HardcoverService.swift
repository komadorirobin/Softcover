  ...
  static func fetchWantToRead(limit: Int) async -> [BookProgress] {
      ...
      do {
          ...
          for ub in rows {
              ...
              let displayTitle = (ub.edition?.title?.isEmpty == false) ? ub.edition!.title! : bookData.title
...
extension HardcoverService {
    ...
    /// Basic implementation orders books by users_count desc to approximate popularity.
    static func fetchTrendingBooksMonthly(limit: Int, imageMaxPixel: Int = 280, compression: CGFloat = 0.75) async -> [TrendingBook] {
        ...
        do {
            ...
            guard let root = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                  ...
                  let dataDict = root["data"] as? [String: Any],
                  let rows = dataDict["books"] as? [[String: Any]] else {
                return []
            }
            ...
            for row in rows {
                ...
                var coverData: Data? = nil
                if let img = (row["image"] as? [String: Any])?["url"] as? String, !img.isEmpty {
                    coverData = await fetchAndResizeImage(from: img)
                }
                items.append(TrendingBook(id: id, title: title, author: author, coverImageData: coverData))
            }
            return items
        } catch {
            return []
